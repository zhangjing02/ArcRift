...
export const ConnectView: React.FC = () => {
  ...
  const chronosMcpConfig = `{
  "mcpServers": {
    "chronosmind": {
      "command": "C:\\\\Program Files\\\\nodejs\\\\node.exe",
      "args": ["d:/Devs/ArcRift/backend/dist/mcp/server.js"],
      "env": {
        "CHRONOSMIND_MCP_MODE": "true",
        "CHRONOSMIND_STORAGE_MODE": "sqlite",
        "SQLITE_DB_PATH": "d:/Devs/ArcRift/backend/ChronosMind.db",
        "NODE_ENV": "production"
      }
    }
  }
  ...
  const defaultPrompt = `在当前项目中启用 ChronosMind 长期记忆与知识图谱工作台。每次完成阶段性任务（Bug 修复、架构设计、API 规范更新）时，请主动调用 chronosmind 的 memory_add 工具将经验存入知识库；在遇到类似问题前，先调用 memory_search 检索历史经验。`;
  ...
  const aiTools: AiToolItem[] = [
    {
      ...
      status: "installed",
      prompt: `你已连接本地 ChronosMind 智能记忆知识库。在编码过程中，只要解决重大 Bug、完成架构决策或收到 'CM' / '存档' 指令，请立即调用 chronosmind MCP 工具持久化到当前项目空间。`,
      mcpJson: chronosMcpConfig,
    },
    {
      id: "cursor",
      name: "Cursor",
      ...
      status: "installed",
      prompt: `已连接 ChronosMind 记忆工作台。请在完成重要编码后，自动使用 memory_add 记录问题根因与解决方案。`,
      mcpJson: chronosMcpConfig,
    },
    {
      id: "windsurf",
      ...
      status: "copy_mcp",
      prompt: `你已接入 ChronosMind 知识库。在开发当前工程时，请主动沉淀关键技术决策并利用 memory_search 辅助排障。`,
      mcpJson: chronosMcpConfig,
    },
    {
      id: "claude-code",
      ...
      status: "copy_prompt",
      prompt: `启用 ChronosMind 记忆工作台。在当前终端工程中，通过 stdio MCP 自动维护每日工作简报与关键技术决策。`,
      mcpJson: chronosMcpConfig,
    },
    {
      id: "claude-desktop",
      ...
      status: "copy_mcp",
      prompt: `已加载 ChronosMind MCP 知识库。请在需要时调用 search_memory 检索项目历史背景。`,
      mcpJson: chronosMcpConfig,
    },
    {
      id: "vs-code",
      ...
      status: "copy_mcp",
      prompt: `连接 ChronosMind 记忆库。在任务完成时保存关键决策，遇到未知报错先检索历史解决方案。`,
      mcpJson: chronosMcpConfig,
    },
  ];
        ...
        <div className="nl-connect-hero-left">
          ...
          <div>
            ...
            <p className="nl-hero-desc">
              让 <strong>Google Antigravity</strong>、<strong>Cursor</strong>、<strong>Windsurf</strong>、<strong>Claude Code</strong> 等主流 AI 工具无缝沉淀上下文与经验到 ChronosMind 本地知识库。
            </p>
          </div>
        </div>
      </div>
              ...
              <div key={tool.id} className="nl-tool-card">
                <div className="nl-tool-card-top">
                  ...
                  {tool.status === "installed" ? (
                    <span className="nl-pill-status-green">✓ 已连接</span>
                  ) : (
                ...
                <div className="nl-tool-card-footer">
                  {tool.mcpJson && (
                    ...
                    >
                      {copiedId === `${tool.id}_mcp` ? "✓ 已复制 MCP 配置" : "📋 复制 MCP 配置"}
                    </button>
                  )}
                  <button
        ...
        <div className="nl-connect-panel">
          <div className="nl-card">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <div>
                <h3 style={{ fontSize: 15, fontWeight: 600 }}>ChronosMind 标准 stdio MCP 配置</h3>
                <p style={{ fontSize: 13, color: "var(--nl-text-muted)", marginTop: 4 }}>
                  粘贴到 Antigravity（<code>~/.gemini/config/mcp_config.json</code>）、Cursor（<code>~/.cursor/mcp.json</code>）或 Claude Desktop（<code>claude_desktop_config.json</code>）中即可。
                </p>
              </div>
              <button
                className="nl-btn-primary"
                onClick={() => handleCopy(chronosMcpConfig, "global_mcp")}
              >
                {copiedId === "global_mcp" ? "✓ 已复制 JSON" : "📋 一键复制配置"}
              </button>
            </div>
          ...
          <div className="nl-card">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <div>
                ...
                <p style={{ fontSize: 13, color: "var(--nl-text-muted)", marginTop: 4 }}>
                  可粘贴到任意项目的 <code>.cursorrules</code>、<code>GEMINI.md</code> 或 <code>CLAUDE.md</code> 中，AI 即可全自动为你归档知识。
                </p>
              </div>
              <button
                className="nl-btn-primary"
                onClick={() => handleCopy(defaultPrompt, "global_prompt")}
              >
            ...
            <div style={{ marginTop: 16 }}>
              ...
              <ul style={{ fontSize: 13, color: "var(--nl-text-secondary)", lineHeight: 1.8, paddingLeft: 18 }}>
                <li><code>CM</code> 或 <code>cm</code>：极简双字母，立即提炼当前上下文沉淀到当前 Space</li>
