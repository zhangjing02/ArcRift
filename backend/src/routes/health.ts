...
// GET /api/health/import-bebebus
router.get("/import-bebebus", async (_req: Request, res: Response) => {
  try {
    const { memoryStore, sessionStore, graphStore, vectorStore } = await import("../services/storage");
    const { extractTriples } = await import("../services/extractor");
    const { slidingWindowChunks } = await import("../services/chunker");

    const spaceId = "BeBeBus";
    let session = await sessionStore.getSession(spaceId);
    if (!session) {
      session = await sessionStore.getSessionByName(spaceId);
      if (!session) {
        session = await sessionStore.createSession(spaceId, "mcp", undefined, spaceId);
      }
    }
   ...
   * 断点续播时必须精确携带当前暂停点 positionTime 和 flag=1，确保硬件无缝继续，提供最丝滑的用户听书体验。`
    ...
    for (const it of items) {
      const mem = await memoryStore.createMemory({
        ...
        title: it.title,
        content: it.content,
        importance: it.importance as any,
        unitType: it.unitType as any,
        labels: it.labels,
        tags: it.labels,
        claimStatus: "asserted" as any,
        source: "mcp",
        sourceApp: "ChronosMind",
    ...
    const wm = await memoryStore.saveWorkingMemory({
      sessionId: session._id,
      briefing,
      focusAreas,
      activeDecisions,
    ...
    res.json({
      success: true,
      message: "Successfully imported 4 memories and updated working memory for BeBeBus",
      memories: savedMemories.map(m => ({ id: m.id, title: m.title, unit_type: m.unitType, importance: m.importance })),
      workingMemory: wm
    });
  } catch (err: any) {
    logger.error("Import BeBeBus memories failed:", err);
    res.status(500).json({ error: "Import failed: " + (err?.message || String(err)) });
  }
});
